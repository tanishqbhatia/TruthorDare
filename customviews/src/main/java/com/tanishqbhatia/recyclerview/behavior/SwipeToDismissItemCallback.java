...
class SwipeToDismissItemCallback extends ItemTouchHelper.Callback {
  ...
  public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
    ...
    if (swipeItemPosition != NO_POSITION) {
      swipeItem = customRecyclerView.getCell(swipeItemPosition).getItem();
    }
  ...
  public void onChildDraw(Canvas canvas, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isControlledByUser) {
    ...
    if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
      if (options.getSwipeToDismissCallback() != null && !isItemSettled) {
        options.getSwipeToDismissCallback().onCellSwiping(customRecyclerView, viewHolder.itemView, swipeItem, swipeItemPosition, canvas, dX, dY, isControlledByUser);
      }
  ...
  public void clearView(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
    ...
    if (options.getSwipeToDismissCallback() != null) {
      if (isItemSwiped) {
        options.getSwipeToDismissCallback().onCellDismissed(customRecyclerView, swipeItem, swipeItemPosition);
      } else {
        options.getSwipeToDismissCallback().onCellSettled(customRecyclerView, viewHolder.itemView, swipeItem, swipeItemPosition);
      }
    }
